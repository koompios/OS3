...
const (
	AUR    ContainerType = iota // 0
	APT    ContainerType = iota // 1
...
func (c *Container) GetContainerName() (name string) {
	...
	switch c.containerType {
	case AUR:
		cn.WriteString("pix_managed_aur")
	case APT:
		cn.WriteString("pix_managed_apt")
	case DNF:
		cn.WriteString("pix_managed_dnf")
	case APK:
		cn.WriteString("pix_managed_apk")
	case ZYPPER:
		cn.WriteString("pix_managed_zypper")
	case XBPS:
		cn.WriteString("pix_managed_xbps")
	default:
...
func (c *Container) Exec(capture_output bool, args ...string) (string, error) {
	...
	if !c.Exists() {
		err := c.Create()
		if err != nil {
			log.Default().Println("Failed to initialize the container. Try manually with `pix init`.")
			return "", err
		}
	}
	...
	for linenr := 1; scanner.Scan(); linenr++ {
		if linenr == 2 {
			...
			if text != "# distrobox_binary" {
				if !fail_silently {
					return fmt.Errorf("`~/.local/bin/%s` is not an pix export, refusing to remove.", bin)
				} else {
					return nil
				}
			} else {
