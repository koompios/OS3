...
/*
	func init() {
		log.SetPrefix("\033[1m\033[34m⌬ pix :: \033[0m")
		log.SetFlags(0)
		viper.SetEnvPrefix("pix") // will be uppercased automatically
		viper.AutomaticEnv()
	}
...
*/
func main() {
	pix = cmd.New(Version, fs)

	// root command
	root := cmd.NewRootCommand(Version)
	// root command
	pix.CreateRootCommand(root)
